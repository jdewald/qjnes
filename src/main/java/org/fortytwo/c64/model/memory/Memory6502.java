package org.fortytwo.c64.model.memory;
    ...
    */
    ...
    private int readInternal(int location, int count) {
        ...
        if ((!(((ddrStatus & 0x3) == 0) && gameStatus == 1)) && location >= CHAR_ROM_START && location <= CHAR_ROM_END) {
                ...
                /*Map<Boolean, Integer> locationStatusMap =  Map.of(
                        location >= VIC_START && location <= VIC_END, 0xFF & vic.read(location - VIC_START),
                        location >= COLOR_RAM_START && location <= COLOR_RAM_END, 0xFF & colorRAM.read(location - COLOR_RAM_START),
                        location >= CIA_1_START && location <= CIA_1_END, 0xFF & cia1.read(location - CIA_1_START),
                        location >= CIA_2_START && location <= CIA_2_END, 0xFF & cia2.read(location - CIA_2_START)
                );

                if (locationStatusMap.get(true) != null)
                    return locationStatusMap.get(true);*/

                if (((ddrStatus & CHAR_FLAG) != 0) && location >= VIC_START && location <= VIC_END) {
