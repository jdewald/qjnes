...
public class Mapper extends BaseMemory
{
    ...
    public static Mapper getMapper(int mapper, ROM programROM, ROM charROM){
        if (mapper == 0){
            return new Mapper(programROM, charROM);
        }
        else if (mapper == 1){
            return new MMC1Mapper(programROM, charROM);
        }
        else if (mapper == 2){
            return new UnromMapper(programROM, charROM);
        }
        else if (mapper == 3){
            return new CnromMapper(programROM, charROM);
        }
        else if (mapper == 4){
    ...
    class MappedCharROM extends ROM
    {
        ...
        public int read(int address){
            if (charInvert) { address ^= 0x1000; } // this really should be handled by the MMC3 mapper...
